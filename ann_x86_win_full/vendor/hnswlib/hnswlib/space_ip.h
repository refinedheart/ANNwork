...
InnerProductSIMD16ExtNEON(const void *pVect1v, const void *pVect2v, const void *qty_ptr) {
    ...
    while (pVect1 < pEnd1) {
        float32x4_t a0 = vld1q_f32(pVect1);
        float32x4_t b0 = vld1q_f32(pVect2);
        float32x4_t a1 = vld1q_f32(pVect1 + 4);
        float32x4_t b1 = vld1q_f32(pVect2 + 4);
        float32x4_t a2 = vld1q_f32(pVect1 + 8);
        float32x4_t b2 = vld1q_f32(pVect2 + 8);
        float32x4_t a3 = vld1q_f32(pVect1 + 12);
        float32x4_t b3 = vld1q_f32(pVect2 + 12);
    ...
    return ReduceSumNEON(vaddq_f32(sum01, sum23));
}
...
#if defined(USE_NEON)
static DISTFUNC<float> InnerProductSIMD16Ext = InnerProductSIMD16ExtNEON;
static DISTFUNC<float> InnerProductSIMD4Ext = InnerProductSIMD4ExtNEON;
static DISTFUNC<float> InnerProductDistanceSIMD16Ext = InnerProductDistanceSIMD16ExtNEON;
static DISTFUNC<float> InnerProductDistanceSIMD4Ext = InnerProductDistanceSIMD4ExtNEON;
#else
...
class InnerProductSpace : public SpaceInterface<float> {
 ...
 public:
    ...
    #if defined(USE_NEON)
        InnerProductSIMD16Ext = InnerProductSIMD16ExtNEON;
        InnerProductSIMD4Ext = InnerProductSIMD4ExtNEON;
        InnerProductDistanceSIMD16Ext = InnerProductDistanceSIMD16ExtNEON;
        InnerProductDistanceSIMD4Ext = InnerProductDistanceSIMD4ExtNEON;
    #endif
    #if defined(USE_AVX512)
        if (AVX512Capable()) {
